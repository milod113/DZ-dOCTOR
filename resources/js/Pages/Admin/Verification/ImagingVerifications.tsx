...
export default function ImagingVerifications({ centers }: any) {
    ...
    const handleApprove = () => {
        ...
        if (confirm(`Are you sure you want to verify ${selectedCenter.name}?`)) {
            // Updated route for Imaging
            patch(route('admin.verifications.imaging.update', { imaging: selectedCenter.id, status: 'verified' }), {
                onSuccess: () => setSelectedCenter(null)
            });
        }
    };
    ...
    const handleReject = () => {
        if (!selectedCenter) return;
        // Updated route for Imaging
        patch(route('admin.verifications.imaging.update', {
            imaging: selectedCenter.id,
            status: 'rejected',
    ...
    return (
        <AppLayout>
            <Head title="Imaging Center Verification" />
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                        {centers.data.length === 0 ? (
                            <div className="p-12 text-center text-gray-500">
                                ...
                                <p>No pending imaging center verifications.</p>
                            </div>
                        ) : (
                                        ...
                                        <div className="flex min-w-0 gap-x-4">
                                            ...
                                            <div className="min-w-0 flex-auto">
                                                ...
                                                <div className="mt-1 flex text-xs leading-5 text-gray-500">
                                                    <p className="truncate">License: {center.license_number}</p>
                                                    <span className="mx-2">•</span>
                                                    <p>{center.city}</p>
                                                </div>
                                                <div className="mt-1 flex items-center gap-2 text-xs text-gray-500">
                                                    <Clock className="w-3 h-3" />
                                                    Submitted: {new Date(center.created_at).toLocaleDateString()}
                                                </div>
                                            </div>
                                        </div>
